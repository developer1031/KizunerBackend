...
class TimelineRepository implements TimelineRepositoryInterface
{
    ...
    public function getTimeline(string $userId, array $followList, int $perPage, string $type = null)
    {
        ...
        if ($type) {
            ...
            /*
            $timeline->where(function($query) {
                $query->where('feed_timelines.type', 'status');
                $query->orWhere('feed_timelines.type', 'help');
                $query->orWhere('feed_timelines.type', 'hangout');
            });
            $timeline->where(function($query) use ($skills) {
                $query->where('feed_timelines.type', 'hangout')
                        ->whereNotIn('skillables.skill_id', $skills)
                        ->orWhereNull('skillables.skill_id');
            });
            $timeline->where(function($query) use ($followList) {
                $query->whereIn('feed_timelines.user_id', $followList);
            });
            */
        ...
        //         $query1->where('h.is_completed', 0)
        //         ->whereNull('help.is_completed');
        //     });
        //     $query->orWhere(function($query1) {
        //         $query1->whereNull('help.is_completed')
        //         ->where('h.is_completed',);
        //     });
        // });
