...
{
    private $session, $token, $baseUrl, $ipnSecret;

    function __construct()
    {
        $this->token = config('services.now_payments.now_payments_api_key');
        $this->baseUrl = config('services.now_payments.now_payments_api_url');
        $this->ipnSecret = config('services.now_payments.now_payments_ipn_secret');
    ...
    private function Call($method, $endpoint, $data = [], $isAuthenticate = '')
    {
        $ch = $this->session;
        ...
        switch ($method) {
            case 'GET':
                ...
                if (!empty($data)) {
                    if (is_array($data)) {
                        ...
                        curl_setopt($ch, CURLOPT_URL, $this->baseUrl . $endpoint . '?' . $parameters);
                    } else {
                        if ($endpoint == 'payment') curl_setopt($ch, CURLOPT_URL, $this->baseUrl . $endpoint . '/' . $data);
                    }
                } else {
            ...
            case 'POST':
                ...
                if ($bearerToken != '') {
                    curl_setopt($ch, CURLOPT_HTTPHEADER, ['X-API-KEY: ' . $this->token, 'Content-Type: application/json', 'Authorization: Bearer ' . $bearerToken]);
                } else {
        ...
        if (isset($jsonRes->statusCode) && ($jsonRes->statusCode < 200 || $jsonRes->statusCode > 299)) {
    ...
    {
        return $this->Call('POST', 'payout', $params, true);
    }

    public function verifyPayout(string $batchWithdrawalId, array $params)
    {
        return $this->Call('POST', 'payout/' . $batchWithdrawalId . '/verify', $params, true);
    }
    ...
    public function authenticate()
    {
        ...
        $data = json_encode(
            [
                "email" => Config::getConfigVal('now_payments_email'),
                "password" => Config::getConfigVal('now_payments_password')
            ]
        );
        curl_setopt($ch, CURLOPT_HTTPHEADER, ['X-API-KEY: ' . $this->token, 'Content-Type: application/json']);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
        curl_setopt($ch, CURLOPT_URL, $this->baseUrl . 'auth');
